...
class BaseLerobotDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Keep causal source coordinates available to episode-aware wrappers.
        # Query fields such as ``frame_index`` are returned for every timestamp
        # in the window; the first value is the window start.
        for key in ("episode_index", "frame_index", "index", "task_index"):
            if key in lerobot_sample:
                value = lerobot_sample[key]
