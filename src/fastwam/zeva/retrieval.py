...
class MemoryBank:
    ...
    def add(
        ...
        *,
        ...
        if valid:
            ...
            self._rows.append({"phase": F.normalize(phase.detach().float().cpu(), dim=0), "effect": F.normalize(effect.detach().float().cpu(), dim=0), "episode_id": str(episode_id), "task_id": task_id, "attempt_id": int(attempt_id), "transition_index": int(transition_index), "episode_step": None if episode_step is None else int(episode_step), "window_index": None if window_index is None else int(window_index)})
    ...
    def retrieve(self, query_phase: Tensor, *, episode_id: str | None = None, task_id: int | str | None = None, top_k: int | None = None) -> RetrievalResult:
        ...
        for i, row in enumerate(rows[:k]):
            ...
            sources.append({key: row[key] for key in ("episode_id", "task_id", "attempt_id", "transition_index", "episode_step", "window_index")})
