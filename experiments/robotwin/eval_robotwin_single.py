...
def main(cfg: DictConfig):
    ...
    zeva_enabled = bool(cfg.model.get("zeva", {}).get("enabled", False))
    sim_cfg_name = "sim_robotwin_zeva.yaml" if zeva_enabled else "sim_robotwin.yaml"
    ...
    fixed_seed = cfg.EVALUATION.get("fixed_seed")
    _append_override(overrides, "seed", cfg.seed if fixed_seed is None else fixed_seed)
    ...
    _append_override(
        overrides,
        "eval_num_episodes",
        1 if fixed_seed is not None else cfg.EVALUATION.eval_num_episodes,
    )
    ...
    # Zeva policy arguments are explicit so the delegated RoboTwin process
    # receives the selected memory mode and matching checkpoints.
    for key in ("zeva_mode", "cte_checkpoint", "addon_checkpoint", "pim_top_k", "max_attempts"):
        if key in cfg.EVALUATION:
            value = cfg.EVALUATION[key]
            if key in {"cte_checkpoint", "addon_checkpoint"}:
                value = _resolve_optional_path(value, base=PROJECT_ROOT)
            _append_override(overrides, key, value)
    if fixed_seed is not None:
        _append_override(overrides, "fixed_seed", fixed_seed)
